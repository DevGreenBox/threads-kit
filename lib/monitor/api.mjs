...
import { загрузитьEnv } from '../env.mjs';

// Токен документация велит класть в .env, а Node такие файлы сам не читает.
// Без этой строки человек делает всё по инструкции и получает «нет токена».
загрузитьEnv();
...
// ───────────────── проверка доступа из терминала ─────────────────
// Это библиотека: обычных команд у неё нет, её использует monitor.mjs.
// Но прямой запуск не должен молчать. Он делает одну вещь, ради которой
// сюда и приходят руками: говорит, работает ли токен и какие разрешения
// на самом деле выданы.
//
//   node lib/monitor/api.mjs            проверить токен и лимиты
//   node lib/monitor/api.mjs --help     справка
//
// ⚠️ Главная ловушка Threads: без одобренного скоупа threads_keyword_search
// поиск НЕ выдаёт ошибку — он молча ищет только по вашим собственным постам.
// Отличить это от «ничего не нашлось» нельзя, поэтому проверка делает
// контрольный поиск по заведомо частому слову и говорит, что увидела.

const СПРАВКА_API = `api.mjs — клиент Threads API (библиотека).

  node lib/monitor/api.mjs          проверить токен, лимиты и разрешения
  node lib/monitor/api.mjs --help   эта справка

Нужен токен в переменной THREADS_ACCESS_TOKEN (или в .env).
Как его получить — config/README.md. Что API умеет — docs/api-facts.md.

Проверка делает три безобидных запроса на чтение: кто я, какие лимиты
публикации, и контрольный поиск. Ничего не публикует.
...
async function проверитьДоступ() {
  if (!process.env.THREADS_ACCESS_TOKEN) {
    console.error(
      'Нет переменной THREADS_ACCESS_TOKEN.\n\n' +
        'Токен — это пароль от вашего аккаунта для программы. Положите его в файл .env:\n' +
        '  cp .env.example .env\n' +
        'и впишите значение. Как получить токен — config/README.md.',
    );
    return 2;
  }

  const клиент = new КлиентThreads({ userId: 'me' });
  let провалов = 0;
  console.log('\n=== ПРОВЕРКА ДОСТУПА К THREADS API ===\n');

  try {
    const лимиты = await клиент.getPublishingLimit();
    const d = лимиты?.data?.[0] || лимиты || {};
    console.log('✓ Токен принят площадкой.');
    const постов = d.quota_usage ?? d.reply_quota_usage ?? null;
    if (постов !== null) console.log(`  Израсходовано из суточной квоты: ${постов}`);
  } catch (e) {
    console.error(`✗ Токен не работает: ${scrubToken(e?.message || String(e))}`);
    console.error('  Токен живёт 60 дней, и доступ отзывается после 28 дней простоя.');
    return 1;
  }

  console.log('\nКонтрольный поиск (проверяем скоуп threads_keyword_search):');
  try {
    const найдено = await клиент.keywordSearch({ q: 'shop', searchType: 'TOP', limit: 10 });
    const посты = найдено?.data || [];
    const чужие = посты.filter((п) => п.username && п.username !== клиент.userId);
    if (!посты.length) {
      console.log('  ⚠️ Поиск ничего не вернул.');
      console.log('     Это может значить и «нет одобрения скоупа», и «правда ничего нет».');
      console.log('     Отличить нельзя: площадка в обоих случаях отвечает пустым списком.');
      провалов++;
    } else {
      console.log(`  ✓ Поиск вернул ${посты.length} постов, из них чужих: ${чужие.length}.`);
      if (!чужие.length) {
        console.log('     ⚠️ Все найденные посты — ваши собственные. Это признак того,');
        console.log('     что скоуп threads_keyword_search НЕ одобрен: без него поиск');
        console.log('     молча ограничивается вашим аккаунтом. Мониторинг работать не будет.');
        провалов++;
      }
    }
  } catch (e) {
    console.error(`  ✗ Поиск отклонён: ${scrubToken(e?.message || String(e))}`);
    провалов++;
  }

  console.log(
    провалов === 0
      ? '\nИТОГ: доступ есть, мониторинг можно запускать.\n'
      : '\nИТОГ: доступ неполон. Смотрите config/README.md, раздел про App Review.\n',
  );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (арг.includes('--help') || арг.includes('-h')) {
    console.log(СПРАВКА_API);
    process.exit(0);
  }
  process.exit(await проверитьДоступ());
}
